...
OnlineAlignmentOptions make_options() {
  ...
  const auto configure_gate =
      [&](CandidateStateGroup group, const Eigen::Vector3d &max_abs_error,
        ...
        gate.history_length_updates = 3;
        gate.min_supported_updates = 3;
        gate.required_post_feedback_stable_updates = 1;
...
void test_rejections_and_causal_recovery() {
  ...
  require(!candidate.result.released_to_openvins &&
          ...
          "AlignmentCandidate must retain its bounded solve snapshot and provenance");
  ...
  require(std::isfinite(
              result.diagnostics.candidate_visual_reprojection_p50_px) &&
              std::isfinite(
                  result.diagnostics.candidate_visual_reprojection_p95_px) &&
              std::isfinite(result.diagnostics
                                .candidate_visual_reprojection_trend_pxps) &&
              std::isfinite(result.diagnostics
                                .candidate_covariance_normalized_residual),
          "bounded validation must report monocular P50/P95, trend, and "
  ...
  require(result.diagnostics.candidate_fc_attitude_evaluation_only,
          "FC/body attitude disagreement must be evaluation-only because "
          "turning can transiently flex the Velcro-mounted board");
  ...
  for (const auto &state : result.diagnostics.state_observability) {
    ...
    if (state.state == "attitude")
      require(state.fc_jacobian_norm <= 1.0e-10,
              "FC attitude must remain evaluation-only in the initial graph");
  }
...
void test_turn_flex_attitude_is_evaluation_only() {
  OnlineAlignmentOptions options = make_options();
  // These deliberately impossible attitude gates prove that the FC/body
  // attitude disagreement is not a hidden navigation release condition.
  ...
  require(result.diagnostics.candidate_fc_imu_rotation_residual_deg > 5.0 &&
          ...
          "large transient FC/body attitude disagreement must remain visible "
          "as an evaluation diagnostic only");
}
...
int main() {
  ...
  test_turn_flex_attitude_is_evaluation_only();
