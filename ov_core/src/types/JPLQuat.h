 ...
 */
// =============================================================================
// [中文] JPLQuat — JPL 约定下的单位四元数 Type 对象 (姿态表示)
//
// 名义维度 _value = 4  ([q1, q2, q3, q4], q4 为标量, 满足 |q|=1)
// 误差维度 _size  = 3  (δθ, 轴角形式)
// 注入方式: 左乘扰动 q_new = dq ⊗ q_old,  dq ≈ [0.5 δθ; 1] (对 SO(3) 的标准小角度近似)
// 与 Hamilton 约定的区别: 四元数乘法和旋转方向都差一个转置, 具体见 quat_ops.h 顶部说明。
//
// 左乘误差与 SO(3) 误差是等价的 (Trawny Eq.71):
//   R(q_new) ≈ exp(-δθ) * R(q_old)  ⟺   q_new ≈ [0.5δθ; 1] ⊗ q_old
// OpenVINS 全部用左乘; 如果自己接入新模块, 别混用右乘扰动。
// =============================================================================
class JPLQuat : public Type {

public:
  // [中文] 默认值: q = (0,0,0,1) 代表单位旋转 (I_3); FEJ 同步初始化到同一值。
  JPLQuat() : Type(3) {
